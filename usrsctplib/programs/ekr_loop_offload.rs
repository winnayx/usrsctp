...
/* Default simple round-robin */
    ...
    #[no_mangle]
    fn usrsctp_enable_crc32c_offload();
    #[no_mangle]
    fn usrsctp_crc32c(_: *mut libc::c_void, _: size_t) -> uint32_t;
    #[no_mangle]
...
unsafe extern "C" fn handle_packets(mut arg: *mut libc::c_void)
 -> *mut libc::c_void {
    ...
    let mut dump_buffer: *mut libc::c_char = 0 as *mut libc::c_char;
    let mut hdr: *mut sctp_common_header = 0 as *mut sctp_common_header;
    let mut length: ssize_t = 0;
    let mut buffer: [libc::c_char; 65536] = [0; 65536];
    let mut received_crc32c: uint32_t = 0;
    let mut computed_crc32c: uint32_t = 0;
    fdp = arg as *mut libc::c_int;
    loop  {
        length =
            recv(*fdp, buffer.as_mut_ptr() as *mut libc::c_void,
                 ((1 as libc::c_int) << 16 as libc::c_int) as size_t,
                 0 as libc::c_int);
        if length > 0 as libc::c_int as libc::c_long {
            dump_buffer =
                usrsctp_dumppacket(buffer.as_mut_ptr() as *const libc::c_void,
                                   length as size_t, 0 as libc::c_int);
            if !dump_buffer.is_null() {
                /* fprintf(stderr, "%s", dump_buffer); */
                usrsctp_freedumpbuffer(dump_buffer);
            }
            if length as size_t >=
                   ::std::mem::size_of::<sctp_common_header>() as
                       libc::c_ulong {
                hdr = buffer.as_mut_ptr() as *mut sctp_common_header;
                received_crc32c = (*hdr).crc32c;
                (*hdr).crc32c = htonl(0 as libc::c_int as uint32_t);
                computed_crc32c =
                    usrsctp_crc32c(buffer.as_mut_ptr() as *mut libc::c_void,
                                   length as size_t);
                ...
                if received_crc32c == computed_crc32c {
                    ...
                    fprintf(stderr,
                            b"Wrong CRC32c: expected %08x received %08x\n\x00"
                                as *const u8 as *const libc::c_char,
                            ntohl(computed_crc32c), ntohl(received_crc32c));
                }
            } else {
                fprintf(stderr,
                        b"Packet too short: length %zu\x00" as *const u8 as
                            *const libc::c_char, length as size_t);
            }
        }
    };
}
unsafe extern "C" fn conn_output(mut addr: *mut libc::c_void,
    ...
    if length >= ::std::mem::size_of::<sctp_common_header>() as libc::c_ulong
       {
        hdr = buffer as *mut sctp_common_header;
        (*hdr).crc32c = usrsctp_crc32c(buffer, length)
    }
    dump_buffer = usrsctp_dumppacket(buffer, length, 1 as libc::c_int);
    if !dump_buffer.is_null() {
        /* fprintf(stderr, "%s", dump_buffer); */
        usrsctp_freedumpbuffer(dump_buffer);
    }
...
unsafe extern "C" fn receive_cb(mut sock: *mut socket,
 ...
 -> libc::c_int {
    printf(b"Message %p received on sock = %p.\n\x00" as *const u8 as
               *const libc::c_char, data, sock as *mut libc::c_void);
    if !data.is_null() {
        if flags & 0x2000 as libc::c_int == 0 as libc::c_int {
            printf(b"Message of length %d received via %p:%u on stream %u with SSN %u and TSN %u, PPID %u, context %u, flags %x.\n\x00"
                       as *const u8 as *const libc::c_char,
                   ...
                   rcv.rcv_tsn, ntohl(rcv.rcv_ppid), rcv.rcv_context, flags);
        }
        free(data);
 ...
 -> libc::c_int {
    ...
    fd_c =
        socket(2 as libc::c_int, SOCK_DGRAM as libc::c_int,
               IPPROTO_UDP as libc::c_int); /* 1 MB */
    ...
    printf(b"Change send socket buffer size from %d \x00" as *const u8 as
               *const libc::c_char, cur_buf_size);
    snd_buf_size = (1 as libc::c_int) << 20 as libc::c_int;
    ...
    printf(b"to %d.\n\x00" as *const u8 as *const libc::c_char, cur_buf_size);
    s_l =
        usrsctp_socket(123 as libc::c_int, SOCK_STREAM as libc::c_int,
                       IPPROTO_SCTP as libc::c_int,
                       Some(receive_cb as
    ...
    printf(b"Change receive socket buffer size from %d \x00" as *const u8 as
               *const libc::c_char, cur_buf_size);
